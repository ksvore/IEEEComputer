...
Grover's algorithm solves the unstructured search problem on a quantum computer with only $O(\sqrt{N})$ evalutions of $f$ in the worst case, while classically $O(N)$ evaluations are required.  Applications include finding the minimum of an unsorted list of integers, identifying whether a graph is connected, and finding a given pattern within a length of text.

\item     Linear systems:
The task of solving a system of linear equations is given an $N\times N$ matrix $A$ and a $N$-dimensional vector $b$, find $x$ such that $Ax=b$.
Classically, it requires time polynomial in $N$ using methods like Gaussian elimination.
Quantumly, it requires time polynomial in log $N$, albeit there are some constraints on the size of $A$ and its condition number \cite{}. 
...
\item    Sampling:
MATTHIAS - will you add here?

\item       Walks:
A walk, or random walk, is a succession of random steps on a given graph.
A quantum walk involves a coherent evolution to determine each step.
Quantum walks can outperform random walks on tasks including the hitting time, determining the hitting time from a source to destination vertex, and the mixing time, spreading from a source to all vertices in a graph, of certain graphs such as glued trees and the hypercube.  Applications include the evaluation of Boolean formulas and the simulation of Markov chains.

\item       Etc.
MATTHIAS - do you want to talk about annealing applications more explicitly?
